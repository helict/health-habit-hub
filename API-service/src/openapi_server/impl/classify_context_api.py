...
Your task is to analyze a sentence written in {language}. For each of the seven contextual components, you must:
...
- Each object must follow this pattern:
  {{"name": "<one of the seven contextual components>", "value": "<exact snippet from sentence>" | null, "classification": 0 | 1, "confidence": 0.00-1.00}}
...
{{"name":"REASONING","value":null,"classification":0,"confidence":0.95}}
]
...
def _validate_and_parse(output_list: List[dict]) -> Optional[List[Context]]:
    try:
        ...
        if len(output_list) != len(expected_names):
            return None
