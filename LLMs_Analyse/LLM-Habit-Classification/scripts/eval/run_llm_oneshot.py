...
Below are the classifications for each sentence:
...
64. 'Ich habe gestern Pommes gegessen' — **not a habit**
65. 'Ich mag Eis, aber nicht jedes Mal.' — **not a habit**
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — **not a habit**#### Habit Recognition Results
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — **not a habit**"""
    ...
    llm_dict[
        "GPT-4o"
    ].output = """Here is the evaluation of the sentences in the given list, following the specified format:
...
Let me know if you need further clarification! 😊"""
    ...
    ].output = """I'll analyze each sentence to determine whether it describes a recurring or routine behavior (habit) or not. Here's my evaluation:
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    llm_dict[
        "Claude Sonnet 3.5"
    ].output = """I'll evaluate each sentence and classify it as either a "habit" or "not a habit." I'll look for indicators of recurring behavior, routine actions, or regular patterns.
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    ].output = """I'll evaluate each sentence to determine whether it describes a habit (recurring or routine behavior) or not.
...
66. Gestern habe ich zum ersten Mal Yoga gemacht. — not a habit"""
    ...
    llm_dict[
        "Claude Opus 4"
    ].output = """I'll evaluate each sentence to determine whether it describes a habit or not. Here's the analysis:
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    llm_dict[
        "Claude Opus 3"
    ].output = """Here is the list with each sentence classified as either a habit or not a habit:
...
Note that some sentences could be argued to be habits or not habits depending on the context, but I've made my best judgment based on the information provided.
"""
    llm_dict[
        "Mistral Large 2"
...
"""
    llm_dict[
        "Deepseek-V3"
    ].output = """Here is the numbered list with the classification for each sentence:
...
"""
    ...
    ].output = """ Here is the evaluation of each sentence as either a habit or not a habit:
...
df.to_excel("llm_all_info_with_cost_performance_oneshot.xlsx", index=False)

print(df.loc[df["Model"].eq("Llama-3.1-8B")])