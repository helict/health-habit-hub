...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    llm_dict[
        "o3 mini (High Effort)"
    ].output = """Below is the numbered classification of each sentence:
...
26. **'nach dem Abendbrot wasche ich ab'** — habit  
27. **'Nach dem Aufstehen mache ich ein paar Sprachübungen'** — habit  
28. **'Ich prüfe ob ich meinen Schlüssel dabeihabe, wenn ich aus der  WOhung gehe'** — habit  
29. **'Ich nehme VItamin D tabletten ein'** — habit  
30. **'Samstags putze ich das Bad.'** — habit  
31. **'Wenn es nicht regnet, gieße ich im Sommer jeden Tag den Apfelbaum.'** — habit  
32. **'Nach dem Abendbrot an einem stressigen Tag esse ich gern ein Stück Schokolade'** — habit  
...
66. **'Gestern habe ich zum ersten Mal Yoga gemacht.'** — not a habit"""
    ...
    llm_dict[
        "GPT-4o mini"
    ].output = """Here is the evaluation of each sentence with the corresponding label:
...
This classification reflects whether each sentence describes a recurring or routine behavior."""


    llm_dict[
        "GPT-4o"
    ].output = """Here is the classification of the 66 sentences, based on whether they describe a **habit** (a recurring or routine behavior) or **not a habit** (a one-time action or intention):
...
### Summary
- **Habits:** 33 sentences  
- **Not a habit:** 33 sentences  

Let me know if you need further clarification!"""
    llm_dict[
        "Claude Sonnet 4 (Extended)"
    ].output = """Based on the definition of habit as a learned stimulus-response association that generates recurring behaviors, here are my classifications:
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    llm_dict[
        "Claude Sonnet 4"
    ].output = """Based on the definition of habit as a learned stimulus-response association that generates impulses to act, I'll evaluate each sentence:
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    ].output = """Based on the provided definition of habit as a learned stimulus-response association that generates an impulse to act, I've evaluated each sentence to determine if it describes a recurring or routine behavior:
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    llm_dict[
        "Claude Sonnet 3.7"
    ].output = """Based on the definition of habit as a learned stimulus-response association generating an impulse to act, I'll analyze each sentence:
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    llm_dict[
        "Claude Sonnet 3.5"
    ].output = """I'll help evaluate these sentences to determine which ones describe habits. I'll analyze them based on whether they represent recurring/routine behaviors with established stimulus-response patterns.
...
66. 'Gestern habe ich zum ersten Mal Yoga gemacht.' — not a habit"""
    ...
    ].output = """I'll analyze each sentence to determine if it describes a habit based on the definition provided. Here's my evaluation:
...
66. Gestern habe ich zum ersten Mal Yoga gemacht. — not a habit"""
    ...
    llm_dict[
        "Claude Opus 4"
    ].output = """Here is my evaluation of each sentence to determine whether it describes a habit:
...
"""
    llm_dict[
        "Mistral Large 2"
    ].output = """Sure, here are the classifications for each sentence:
...
"""
    llm_dict[
        "Deepseek-V3"
    ].output = """Here is the labeled list of sentences, evaluated based on the definition of a habit as a learned stimulus-response association that generates recurring or routine behavior:  
...
### Key Observations:  
- **Habits** are marked by **repetition, routine, or automatic responses to a stimulus** (e.g., "nach dem Aufstehen," "immer wenn," "wenn X, dann Y").  
- **Not habits** include **one-time actions, plans, preferences, or non-recurring events** (e.g., "gestern," "zum ersten Mal," "ich plane").  
- Some borderline cases (e.g., "Ich jogge") were labeled **"not a habit"** because they lack a clear **stimulus-response pattern** or **frequency indicator** (compare with "Nach dem Joggen trinke ich einen Proteinshake," which is habit-forming).  
...
Here is the evaluation of each sentence based on the criteria for habit recognition:
...
**Key Observations**:  
- **Habit sentences** consistently describe **repetitive, context-triggered actions** (e.g., morning routines, stress responses, recurring weekly tasks).  
- **Non-habit sentences** describe **one-time events, preferences, plans, or states** (e.g., "gestern im Kino," "Ich liebe Pommes").  
- Ambiguous cases (e.g., "Ich jogge") were classified as habits due to the generalized present tense implying routine.
"""
...
df.to_excel("llm_all_info_with_cost_performance_with_definition.xlsx", index=False)
